...
public class Package implements Feature {
    ...
    public ExecutionReport execute(Project project, Object... params) {
        System.out.println("Running Maven Package");
        return () -> {
            try {
                Process process = Runtime.getRuntime().exec("mvn package", null,
                ...
                if (exitCode == 0) {
                    System.out.println("Maven package successful!");
                } else {
                    System.out.println("Maven package failed. Exit code: " + exitCode);
                }
            } catch (Exception e) {
                e.printStackTrace();
                return false;
